...
/*  FlexLife role-play — live chat.
    ...
    annotated; any prospect message can be flagged and rewritten. Export gives
...
const SEED = {
  ...
  persona_id: "P02",
  persona_name: "The Family Balancer",
...
export default function RolePlayChat() {
  ...
  const brief = `PERSONA: ${p.persona_name} (${p.persona_id}), difficulty ${p.difficulty}
...
Return ONLY JSON: {"text":"...","state":"snake_case","open":"short phrase or None","end":"in_progress | near_complete | no_more_questions"}`
    );
  }
  ...
  async function retry(id) {
    ...
    try {
      ...
      setMsgs((prev) =>
        prev.map((m, j) =>
          j === i
            ? { ...m, text: out.text, original: out.text, state: out.state || "", open: out.open || "", end: out.end || "in_progress", flag: null }
            : m
        )
      );
    } catch (e) {
...
Return ONLY JSON: {"text":"..."}`
  ...
  function csv() {
    const cols = ["conversation_id","persona_id","persona_name","difficulty","scene","known_background","hidden_customer_state","training_objective","plan_protection_gaps","plan_recommendation","plan_budget_tradeoffs","turn_number","speaker","utterance","rep_behavior_label","customer_state_after_turn","unresolved_questions","gold_response_requirements","end_state","sme_note","sme_reading","sme_intent","sme_held_back","utterance_origin","realism_flag","flag_reason","ai_original_utterance"];
    const esc = (v) => `"${String(v ?? "").replace(/"/g, '""')}"`;
    let turn = 0;
    const rows = msgs.map((m, i) =>
      [p.conversation_id, p.persona_id, p.persona_name, p.difficulty,
        i === 0 ? "post_fact_find_recommendation_meeting" : "",
        ...
        m.open || "",
        m.speaker === "rep" ? m.req || "" : m.speaker === "customer" ? "Customer turn; no evaluation" : "",
        m.speaker === "thinking" ? "" : m.end || "in_progress",
        m.speaker === "thinking" ? m.text : m.note || "",
        m.reasoning?.read || "", m.reasoning?.why || "", m.reasoning?.held || "",
        m.speaker === "customer" ? (m.origin === "sme" ? "sme_written" : "ai_generated") : "",
        m.flag ? "unrealistic_corrected" : "",
        m.flag?.reason || "", m.flag && m.original !== m.text ? m.original : "",
      ].map(esc).join(",")
    );
    ...
    <div className="flex h-screen flex-col bg-slate-50 text-[14px] text-slate-900">
      ...
      <header className="flex shrink-0 items-center gap-3 border-b border-slate-200 bg-white px-4 py-2.5">
        ...
        <div className="min-w-0">
          ...
          <p className="truncate text-[12px] text-slate-500">
            {typing ? "typing…" : msgs.length ? msgs[msgs.length - 1].state || "in conversation" : `${p.difficulty} · FlexLife`}
          </p>
        </div>
        <div className="ml-auto flex items-center gap-1.5">
          <button onClick={() => setSetupOpen(true)} className="rounded px-2.5 py-1.5 text-[13px] text-slate-600 hover:bg-slate-100">Brief</button>
          <button onClick={copyRows} disabled={!msgs.length} className="rounded px-2.5 py-1.5 text-[13px] text-slate-600 hover:bg-slate-100 disabled:opacity-40">{copied ? "Copied" : "Copy"}</button>
          <button onClick={download} disabled={!msgs.length} className="rounded bg-teal-700 px-3 py-1.5 text-[13px] text-white hover:bg-teal-800 disabled:opacity-40">Export</button>
        </div>
      </header>
          ...
          {!msgs.length && !typing && (
            <div className="mx-auto max-w-lg py-6">
              <p className="text-[13px] leading-relaxed text-slate-600">
                The fact-find is done and you're turning the conversation toward FlexLife.
                Work out how it applies to her specifically — then pick the conversation up
                where she reacts to it.
              </p>
              <div className="mt-5 rounded-lg border border-slate-200 bg-white p-4">
                {planBlock}
                <p className="text-[11px] leading-snug text-slate-400">
                  She can't see this. It shapes the drafts you ask for and travels with the
                  transcript on export.
                </p>
              </div>
              <div className="mt-4">
                <button
                ...
                <p className="mt-2 text-[11px] text-slate-400">
                  {p.persona_name} speaks next. You land mid-conversation, not at hello.
                </p>
              </div>

              <div className="mt-5 rounded-lg border border-slate-200 bg-white p-4">
                <p className="mb-1.5 text-[12px] font-medium text-slate-700">
                  Or put her first line in yourself
                </p>
                <textarea
                  rows={3}
                  value={ownOpener}
                  onChange={(e) => setOwnOpener(e.target.value)}
                  placeholder="Write the line you've actually heard at this point in a real presentation"
                  className="w-full rounded border border-slate-300 px-2 py-1.5 text-[13px] leading-snug outline-none focus:border-teal-600"
                />
                <button
              ...
              <div key={m.id} className={`mb-4 flex ${rep ? "justify-end" : "justify-start"}`}>
                <div className={`max-w-[85%] ${rep ? "items-end" : "items-start"} flex flex-col`}>
                      ...
                      <textarea
                        ...
                        onBlur={(e) =>
                          e.target.value.trim() && e.target.value !== m.text
                            ? commitRewrite(m.id, { text: e.target.value, origin: "sme", editing: false })
                            : patch(m.id, { editing: false })
                        }
                        className="w-full resize-y rounded border border-teal-300 px-2 py-1 leading-relaxed outline-none"
                      />
                    ) : (
                      m.text
                    )}
                  ...
                  <div className={`mt-1 flex flex-wrap items-center gap-2.5 px-1 text-[11px] text-slate-400 ${rep ? "justify-end" : ""}`}>
                    {m.flag && <span className="text-amber-600">corrected</span>}
                    {!rep && m.origin === "sme" && <span className="text-teal-700">your wording</span>}
                    {!rep && m.state && <span>{m.state}</span>}
                    <button onClick={() => setOpenNote(openNote === m.id ? null : m.id)} className="hover:text-teal-700">
                      {m.note ? "note" : "+ note"}
                    </button>
                    {!rep && (
                      <>
                        <button onClick={() => { setOpenFlag(null); patch(m.id, { editing: !m.editing }); }} className="hover:text-teal-700">{m.editing ? "done" : "edit"}</button>
                        <button onClick={() => setOpenFlag(openFlag === m.id ? null : m.id)} className="hover:text-amber-600">not realistic</button>
                        <button onClick={() => retry(m.id)} disabled={typing} className="hover:text-teal-700 disabled:opacity-40">retry</button>
                      </>
                    )}
                    {rep && (
                      <button onClick={() => setInput(m.text)} className="hover:text-teal-700">reuse</button>
                    )}
                  </div>
                  ...
                  {openFlag === m.id && (
                    <div className="mt-1 w-full max-w-md rounded-lg border border-amber-200 bg-amber-50 p-2.5">
                      <textarea rows={2} defaultValue={m.flag?.reason || ""}
                        onBlur={(e) => patch(m.id, { flag: { ...(m.flag || {}), reason: e.target.value } })}
                        placeholder="Why a real prospect wouldn't say this"
                        className="mb-1.5 w-full rounded border border-amber-200 bg-white px-2 py-1.5 text-[12px] outline-none focus:border-amber-500" />
                      <textarea rows={3} defaultValue={m.text}
                        onBlur={(e) =>
                          e.target.value.trim() && e.target.value !== m.text
                            ? commitRewrite(m.id, {
                                text: e.target.value,
                                origin: "sme",
                                flag: { ...(m.flag || {}), reason: m.flag?.reason || "" },
                              })
                            : patch(m.id, { flag: { ...(m.flag || {}), reason: m.flag?.reason || "" } })
                        }
                        className="mb-1 w-full rounded border border-amber-200 bg-white px-2 py-1.5 text-[13px] leading-relaxed outline-none focus:border-amber-500" />
                      <p className="mb-1.5 text-[10px] leading-snug text-amber-700">
                        Your version replaces hers and the conversation carries on from it.
                        Anything said after this point is removed.
                      </p>
                      <div className="flex items-center gap-2">
                        <select value={m.state || ""} onChange={(e) => patch(m.id, { state: e.target.value })}
                          className="rounded border border-amber-200 bg-white px-1.5 py-1 text-[11px]">
                          <option value="">state after turn</option>
                          {STATES.map((s) => <option key={s} value={s}>{s}</option>)}
                        </select>
                        <button onClick={() => setOpenFlag(null)} className="text-[11px] text-amber-700 underline">done</button>
                      </div>
                    </div>
                  )}
                </div>
      ...
      <div className="shrink-0 border-t border-slate-200 bg-white px-4 py-3">
        <div className="mx-auto max-w-2xl">
          {noteOpen ? (
            <div className="mb-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2.5">
              {[
                ["read", "What you're reading in her right now"],
                ["why", "Why you're going where you're going"],
                ["held", "What you considered and decided against"],
              ].map(([k, label]) => (
                <label key={k} className="mb-2 block last:mb-0">
                  <span className="mb-0.5 block text-[10px] text-amber-700">{label}</span>
                  <textarea
                    rows={2}
                    value={think[k]}
                    onChange={(e) => setThink({ ...think, [k]: e.target.value })}
                    className="w-full resize-none rounded border border-amber-200 bg-white px-2 py-1 text-[12px] leading-snug text-amber-900 outline-none focus:border-amber-500"
                  />
                </label>
              ))}
              <div className="mt-1.5 flex items-center gap-3 text-[11px]">
                <span className="text-amber-700">Sends attached to your next message.</span>
                <button onClick={postThinking} className="ml-auto text-amber-800 underline">
                  post on its own
                </button>
                <button
                  onClick={() => { setThink({ read: "", why: "", held: "" }); setNoteOpen(false); }}
            ...
            <button onClick={() => setNoteOpen(true)}
              className="mb-2 text-[11px] text-slate-400 hover:text-amber-700">
              + your thinking
            </button>
          )}
        </div>
        <div className="mx-auto flex max-w-2xl items-end gap-2">
          <button onClick={draftForMe} disabled={drafting || typing}
            className="shrink-0 rounded-full border border-slate-300 px-3 py-2 text-[12px] text-slate-600 hover:border-teal-600 hover:text-teal-700 disabled:opacity-40">
            {drafting ? "…" : "draft"}
          </button>
        ...
        <aside className="hidden w-80 shrink-0 overflow-y-auto border-l border-slate-200 bg-white px-4 py-4 xl:block">
          ...
          >
            {plan.gaps || plan.rec || plan.tradeoffs ? "Your recommendation" : "Write your recommendation"}
          </button>
        </aside>
      </div>
          ...
          <div className="h-full w-full max-w-sm overflow-y-auto bg-white p-5 shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            {msgs.length > 0 && (
              <button onClick={() => { setMsgs([]); setSetupOpen(false); }}
                className="mt-2 text-[12px] text-slate-500 underline hover:text-amber-700">
